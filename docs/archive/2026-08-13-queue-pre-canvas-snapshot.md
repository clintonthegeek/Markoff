...
## #17 — markoff-canvas spike (projection view leaf on QTextLayout)

**Filed 2026-08-13.** Direction decision + spike spec written this
session after a full stack evaluation (dual-authority diagnosis of the
view seam; qtbase fork rejected permanently):

- Decision record: `docs/specs/2026-08-13-view-authority-direction-decision.md`
- Spike spec (falsifiable exit criteria E1–E10, constitutional
  constraints C1–C4, ~3-week timebox):
  `docs/specs/2026-08-13-markoff-canvas-spike-design.md`

A fourth leaf (`libs/markoff-canvas/`) rendering `MarkoffDocument`
directly via per-block `QTextLayout` — no `QTextDocument`, no QML
`TextEdit`, zero re-entrance guards, zero deferrals, one coordinate
space. The spike tests whether the projection architecture eliminates
the guard/defer/coordinate-space bug classes the Discipline Log
catalogs. **While the spike is open: bug fixes only in the live/styled
seam layer** (decision record §5.2). Outcome feeds the D5 design
(pass → markoff-live retirement plan written there; fail → archive
with findings, status quo stands).
