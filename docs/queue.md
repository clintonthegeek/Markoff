...
- 2026-05-21 `libs/markoff-live/qml/LiveView.qml:onPositionChanged` + `LiveCursorState::extend` — inv #4 + #5 — cross-block selection downward is broken in production (drag-down doesn't extend, Shift+Down doesn't extend, Backspace/Delete on selection deletes wrong region). Upward works. Surfaced 2026-05-21 by Corbomite dogfood once session-restore fixes (`c3316f9` + setActiveLeaf loadIfDeferred) made the editor usable for the first time. `tst_live_render_e2_nav_shift_extend_qml::shift_down_at_visual_bottom_extends_selection` passes — synonym not production callsite (programmatic `keyClick` + offscreen QPA bypasses real focus/grab path). Falsifiability-tested against `c3316f9`: reverting it does NOT fix the regression, so `c3316f9` is exonerated. Bug exists pre-this-session in `b31d50a` (LiveSelectionView retirement + migration to LiveCursorState) / `30c4f57` (non-focusing caret chokepoint) era. **Needs a falsifiable invariant test on `LiveRealisticInputHarness` driving real QML mouse + key events before any fix.** Folded into queue #2 as a top-priority concern.
