...
* Pass 3 (Bug 3 v2, this commit): anchor-keyed delivery mis-resolved in some mid-document cases — the captured pre-edit `BlockAnchor` matched a row whose post-edit content was the *originally-following* paragraph, not the user's shifted content. Switched to byte-keyed `requestTextCaretAtByte(document, byte, 0)`. The byte position is unambiguous: the user's content's first byte is exactly `markerBytes` past the original block start. The resolver asks the foundation directly which row's `blockByteRange` contains that byte. This was intended to sidestep anchor renumbering by using parser-blessed byte ranges. **The user reproduced the bug on this commit too**; the failure mode remains unexplained.

> **Status (2026-05-04 dogfood): UNRESOLVED.** The byte-position-keyed approach is the third attempt at correct atStart cursor delivery. All three (row-keyed `requestTextCaretAtNewRow(blockIndex+1)` + bonus check, anchor-keyed `requestTextCaretAtAnchor`, byte-keyed `requestTextCaretAtByte`) reproduce the same dogfood symptom: in a multi-paragraph document like `docs/phase-c-status.md`, the cursor lands on the *originally-following* paragraph instead of the user's shifted content. No synthetic unit test reproduces the failure on any of the three implementations. The bug may not be in `LiveStructuralKeyHandler::paragraphEnter`'s atStart branch at all — candidate downstream sites include QML delegate recycling in `LiveView.qml`, `LiveEditBinding::onContentsChange`'s freshness-rule handling, or how `LiveBlockModel::dataChanged` interacts with the QML `text:` binding mid-parse-back. See `docs/handoff/2026-05-04-bug-3-handoff.md` for the next agent's starting point.
