...
private Q_SLOTS:
    ...
    // backspace_at_start_of_heading_after_hr_demotes_merged_block removed —
    // the merge-and-demote behaviour it asserted was retired by the block-only
    // merge fence (spec §4 R-backspace-at-text-start-adjacent, commit 120311
    // fixing D-fc-4). Backspace at qtPos=0 with a block-only previous block
    // now moves the cursor to BlockSelected on that block instead of merging.
    // New contract is covered by tst_live_render_focus_chokepoint_invariant
    // (R-backspace-at-text-start-adjacent slot).
