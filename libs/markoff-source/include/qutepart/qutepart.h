...
class Qutepart : public QPlainTextEdit {
  ...
  public:
    ...
    ///
    /// Corbomite extension (Phase 2 of the qutepart-corbomite fork, 2026-04-15).
    /// Returns the document block number of the first visible block, plus the
    /// fractional offset of that block above the viewport top expressed as a
    /// fraction of the block's height. Designed for Obsidian-compatible
    /// visual-line float scroll serialization.
    ///
    /// Exposes the otherwise-protected `QPlainTextEdit::firstVisibleBlock` /
    /// `blockBoundingGeometry` / `blockBoundingRect` machinery so the
    /// `Corbomite::SourceEditor` shim can compose the widget (rather than
    /// subclassing or friending it).
    float scrollPositionVisualLine() const;

    /// Sets scroll so that `visualLine` floor becomes the first visible block
    /// and its fractional part determines the pixel offset above the viewport.
