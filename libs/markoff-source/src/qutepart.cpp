...
// ---------------------------------------------------------------------------
// Corbomite-fork Phase 2 (2026-04-15): visual-line float scroll extension.
// Exposes `QPlainTextEdit::firstVisibleBlock` / `blockBoundingGeometry` /
// `blockBoundingRect` (which are protected on QPlainTextEdit) to the
// `Corbomite::SourceEditor` shim via these two public convenience methods.
// Logged in `libs/qutepart-corbomite/PROVENANCE.md` "Modified files".
// ---------------------------------------------------------------------------

float Qutepart::scrollPositionVisualLine() const {
    QTextBlock first = firstVisibleBlock();
    if (!first.isValid()) {
        return 0.0f;
    }
    const qreal blockTop = blockBoundingGeometry(first).translated(contentOffset()).top();
    const qreal blockHeight = blockBoundingRect(first).height();
    if (blockHeight <= 0.0) {
        return static_cast<float>(first.blockNumber());
    }
    // blockTop is measured from viewport top. Negative when the block's top
    // has scrolled above the viewport; fraction is how far past that top we
    // are, as a proportion of the block's height (0 → block's top is at the
    // viewport top; 0.5 → first visible block's top is half a line above the
    // viewport top).
    const float fraction = static_cast<float>(-blockTop / blockHeight);
    return static_cast<float>(first.blockNumber()) + fraction;
}

void Qutepart::setScrollPositionVisualLine(float visualLine) {
    if (visualLine < 0.0f) {
        visualLine = 0.0f;
    }

    // QPlainTextEdit's vertical scrollbar is line-granular: its value is a
    // visual-line index (block first-line + intra-block line), with no
    // sub-line fractional slot. The best we can do on the public API is
    // round `visualLine` to the nearest scrollbar value. `scrollPositionVisualLine()`
    // will still read back sub-line fractions driven by QPlainTextEdit's
    // internal `topLineFracture` (which does populate after some operations),
    // so round-trip is lossy at the fractional layer — stability within
    // ±0.5 visual-line across reflow is the honest contract for Phase 2.
    // Finer sub-line fidelity would require private-API access; the
    // FoldCalculator / KSyntaxHighlighting work in Phase 4 is an
    // opportunity to revisit the scroll model.

    const int targetLine = static_cast<int>(std::round(visualLine));

    QScrollBar *vbar = verticalScrollBar();
    const int clamped = std::clamp(targetLine, vbar->minimum(), vbar->maximum());
